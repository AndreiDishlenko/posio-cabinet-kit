...
> **Оказалось неверно.** Tailwind v3 `content` из пресета не мержит: в
> собранном конфиге остаётся первый найденный `content`, то есть хостовый.
> Glob пакета обязан стоять в конфиге хоста — его туда пишет `cabinet-kit:install`
> и поддерживает `cabinet-kit:sync-config` (см. `docs/CHANGELOG.md`).
