...
Business sections that several hosts need — a product catalog, news — ship as
separate composer packages on top of the kit, not inside it: the shell stays a
shell, and a host installs only what it uses.

- **Discovery.** A module declares `extra.cabinet-kit` in its `composer.json`.
  `Support/CabinetKitModules` (server) and the Vite plugin (build) read the same
  `vendor/composer/installed.json`, so both sides see the same set without any
  registration by hand; `composer remove` takes a module off both.
- **Pages.** Module cabinet pages live in `pages/<module>/` under the folder the
  module names. The plugin exposes them as `virtual:cabinet-kit-modules`, which
  `createCabinetKitApp` merges between the host overrides and the kit's own
  pages; the provider adds the same folders to Inertia's page paths. A glob
  cannot be built at runtime (`import.meta.glob` takes literals only), which is
  why the list is generated by the plugin.
- **Backend.** `CabinetKit` (singleton, facade of the same name) is the registry
  a module's provider fills in `boot()`: route groups under the cabinet stack,
  system permissions (applied by `CabinetKitRoles::sync`), dictionaries (served
  by the kit's own `cabinet-kit.api.dictionaries`, which also folds in a host's
  legacy endpoint), translation folders, sitemap providers, doctor checks and
  sync-config steps. `SeoService` is scoped per request so a controller can
  override the meta of the page it renders.
- **Menu.** Rows in `admin_links`, written by the module's migrations through
  `Support/ModuleMenu`; a module whose routes are gone leaves no broken item
  because `MenuService` hides unresolvable routes.

The authoring contract is in EXTENDING → "Writing a module".
