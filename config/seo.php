...
return [
	...
	// Разработчик сайта (WebSite.creator). Пустые name/url — узел не выводится;
	// id по умолчанию — узел организации на сайте разработчика.
	'site_creator' => [
		'name' => '',
		'url'  => '',
		'id'   => '',
	...
	// Платформа, на которой построен сайт (WebSite.isBasedOn). Пустой id — свойство
	// не выводится; id — узел продукта на сайте платформы.
	'site_platform' => [
		'name' => '',
		'url'  => '',
		'id'   => '',
	...
	// Дата изменения страницы в карте сайта сдвигается, только когда меняется её
	// содержание: мета записи раздела SEO плюс файлы страницы из сборки сайта. Файлы
	// находятся по манифесту сборки: страница маршрута `a.b` — `{pages_dir}A/B.vue`
	// (или `A/b.vue`), вместе с импортами из каталогов содержания. Каркас (раскладки,
	// шапка, общие компоненты) содержанием не считается. Пустой pages_dir — дата
	// следит только за метой.
	'sitemap_lastmod' => [
		'pages_dir'    => '',
		'content_dirs' => [],
		// Манифесты сборки по порядку, относительно public/: первый, где есть страницы сайта.
