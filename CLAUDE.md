...
- **Auth** — вход/регистрация/выход/сброс пароля/подтверждение email —
  реализовано (v0.2.0+).
- **Logs** — просмотр логов приложения через `opcodesio/log-viewer`,
  смонтированный пакетом и закрытый правом `sysper-log-view` — реализовано.
  Собственной страницы у раздела нет: пункт меню ведёт на страницу
  просмотрщика обычной ссылкой.
- **Site settings** — бренд и оформление, которые оператор меняет из кабинета:
  название, значок вкладки, логотипы и тема по умолчанию, раздельно для
  публичной части и кабинета — реализовано.
- **SEO** — постраничная мета, Open Graph, микроразметка JSON-LD и карта сайта
  для публичных страниц хоста — реализовано. Единственные два модуля, которые
  выходят за пределы маршрутов кабинета, поэтому их граница с хостом описана
  отдельно: `docs/ARCHITECTURE.md` → «Site settings and SEO», инструкции для
  потребителя — `docs/host/`.
...
### Пакет — зеркало кабинета posio.cabinet

Пакет — зеркало базы модуля кабинета проекта `posio.cabinet`
(`F:\OpenServer\home\posio.cabinet`) с ограниченным функционалом: в него
попадает только оболочка, без продуктовой части Posio. Источник правды —
`posio.cabinet`, пакет следует за ним, а не наоборот.

- **Правка функционала пакета.** Сначала изменение вносится в
  `posio.cabinet`, затем переносится сюда. Исключение — если пакет подключён
  к `posio.cabinet` как junction: тогда это один и тот же код, и правка
  вносится один раз в пакет.
- **Встречные изменения.** Обнаруженные изменения `posio.cabinet`, которые
  касаются функционала пакета, синхронизируются в пакет, максимально сохраняя
  структуру и исходный код `posio.cabinet` (порядок переноса — ниже).
- **Код без источника в апстриме** (документация для хоста, стабы,
  собственные консольные команды пакета) правится сразу здесь.
...
**Файлы локализации** (`resources/locales/*.js` и подобные) переносятся
из `posio.cabinet` **полностью, без правок** — как есть, без обобщения и
причёсывания под пакет, даже если часть ключей относится к
Posio-специфичной логике. Вырезание лишних ключей — отдельный осознанный
шаг, а не часть переноса.
...
### Точка переноса: смотреть только новое

**Перед любым переносом из `posio.cabinet` прочитай
`tools/upstream-sync-state.json`.** Там записано, до какого релиза апстрима
изменения уже перенесены (`synced_through`: версия, коммит, дата, версия
пакета). Всё, что старше этого коммита, в пакете уже есть — просматривать
надо только то, что появилось после него:

```powershell
.\tools\Sync-CabinetKitFromPosio.ps1 -ShowBaseline   # точка + список новых коммитов
git -C F:\OpenServer\home\posio.cabinet diff --stat <synced_through.commit>..HEAD
```

Аудит (`-OpenDiff`) печатает эту дельту в шапке отчёта сам. Разбор всей
истории проекта вместо дельты — потерянные часы и токены на файлах, которые
не менялись.

После завершённого переноса точка **обязательно** сдвигается:
`.\tools\Sync-CabinetKitFromPosio.ps1 -RecordBaseline` (прежняя уходит в
`history`), и в `synced_through.notes` вписывается одна строка о том, что
именно перенесено.
...
Бизнес-разделы, нужные нескольким хостам (каталог, новости), — отдельные
пакеты-модули поверх кабинета (`posio/catalog-kit` и т.п.), а не код пакета.
Пакет даёт им только точки встраивания (с 0.4): `docs/ARCHITECTURE.md` →
«Modules», контракт — `docs/EXTENDING.md` → «Writing a module». Точку
встраивания, нужную модулю, делать общей для всех модулей, без имён и логики
конкретного.
...
(сервис-провайдер, `loadMigrationsFrom` / `loadRoutesFrom`). Публикуются
(копируются) только конфиги: `config/cabinet-kit.php` и
`config/cabinet-kit-redirects.php`. Любое изменение, требующее от
